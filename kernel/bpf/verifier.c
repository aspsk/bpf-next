...
// XXX all comments look outdated
...
static int visit_goto_void_insn(int t, struct bpf_verifier_env *env, int fd)
{
	...
	map = env_get_map(env, fd, NULL);
	if (IS_ERR(map)) {
		pr_warn("XXX bad map fd=%d: error=%ld\n", fd, PTR_ERR(map));
		return PTR_ERR(map);
	}
...
static int do_misc_fixups(struct bpf_verifier_env *env)
{
	...
	for (i = 0; i < insn_cnt;) {
		...
		if ((insn->code == (BPF_JMP | BPF_JA | BPF_X) ||
			...
			map = env_get_map(env, insn->imm, NULL);
			if (IS_ERR(map))
				return PTR_ERR(map);

			// XXX: rename it and define once!
			struct insn_ptr {
...
static int env_add_map_from_fd_array(struct bpf_verifier_env *env, int fd)
{
	struct bpf_map *map;
	struct fd f;
	int ret;
	...
	/*
	 * fd_array may contain map_fds, btf_fds, and trash.
	 * Ignore everything except maps for now.
	 */
	map = __bpf_map_get(f);
	if (IS_ERR(map))
		return 0;
...
static int env_init_fd_array(struct bpf_verifier_env *env, union bpf_attr *attr, bpfptr_t uattr)
{
	...
	for (i = 0; i < attr->fd_array_cnt; i++) {
		ret = env_add_map_from_fd_array(env, copy[i]);
		if (ret)
			goto free_copy;
	}
