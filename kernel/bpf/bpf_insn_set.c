...
// XXX: cleanup the list of incluides
#include <linux/bpf.h>
#include <linux/btf.h>
#include <linux/err.h>
...
#define ISET_CREATE_FLAG_MASK 0
...
static int insn_set_map_alloc_check(union bpf_attr *attr)
{
	...
	if (attr->max_entries == 0 || attr->key_size != 4 ||
	    attr->value_size != 4 ||
	    attr->map_flags & ~ISET_CREATE_FLAG_MASK)
		return -EINVAL;
...
*/
		...
		for (j = 0; j < map->max_entries; j++)
			if (insn_set->ptrs[i].xlated_off == xlated_off) // XXX subprogs? :hmm:
				return &insn_set->ptrs[i];
	}
...
void bpf_prog_update_jitted_insn_offset(struct bpf_prog *prog, u32 xlated_off, u32 jitted_off, u32 jitted_len, u32 jitted_jump_offset, void *jitted_ip); // XXX where to declare?
