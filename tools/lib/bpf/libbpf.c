...
static int insn_array_update_elem(int map_fd, int i, __u32 xlated_off)
{
	...
	return bpf_map_update_elem(map_fd, &i, &val, 0);
}
					   ...
					   struct bpf_map *map)
{
	...
	for (i = 0; i < key->insns_cnt; i++) {
		err = insn_array_update_elem(map->fd, i, key->insns[i].insn_offset);
		if (err) {
			err = -errno;
			pr_warn("map '%s': failed to set initial contents: %s\n",
				bpf_map__name(map), errstr(err));
...
static int create_jt_map(struct bpf_object *obj, int off, int size, int adjust_off)
{
	...
	for (i = 0; i < attr.max_entries; i++) {
		err = insn_array_update_elem(map_fd, i, jt[2*i]/8 + adjust_off);
		if (err) {
			close(map_fd);
			return err;
		}
