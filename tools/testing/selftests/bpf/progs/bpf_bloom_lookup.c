...
u64 nr_entries;
u64 nr_loops;
u32 __attribute__((__aligned__(8))) key[MAX_NR_CPUS];

/* Filled by us */
u64 __attribute__((__aligned__(256))) percpu_times_index[MAX_NR_CPUS];
u64 __attribute__((__aligned__(256))) percpu_times[MAX_NR_CPUS][NR_SLOTS];
...
{
	patch_key(index);
	return bpf_map_peek_elem(&bloom_map_bench, key) ? 1 : 0;
}
