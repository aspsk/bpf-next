...
#include <argp.h>
#include "bench.h"
#include "bpf_hashmap_new_lookup.skel.h"
#include "bpf_util.h"

/* BPF triggering benchmarks */
static struct ctx {
	struct bpf_hashmap_new_lookup *skel;
} ctx;
...
const struct argp bench_hashmap_new_lookup_argp = {
	.options = opts,
	.parser = parse_arg,
};
...
static void setup(void)
{
	...
	ctx.skel = bpf_hashmap_new_lookup__open();
	if (!ctx.skel) {
	...
	ret = bpf_hashmap_new_lookup__load(ctx.skel);
	if (ret) {
		bpf_hashmap_new_lookup__destroy(ctx.skel);
...
const struct bench bench_bpf_hashmap_new_lookup = {
	.name = "bpf-hashmap-new-lookup",
	.argp = &bench_hashmap_new_lookup_argp,
	.validate = validate,
	.setup = setup,
