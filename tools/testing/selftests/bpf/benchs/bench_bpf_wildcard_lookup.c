...
#include <argp.h>
#include "bench.h"
#include "bpf_wildcard_lookup.skel.h"
#include "bpf_util.h"

/* BPF triggering benchmarks */
static struct ctx {
	struct bpf_wildcard_lookup *skel;
} ctx;
...
static const struct argp_option opts[] = {
	{ "key_size", ARG_KEY_SIZE, "KEY_SIZE", 0,
	  "The wildcard key size (max 1024)"},
	{ "map_flags", ARG_MAP_FLAGS, "MAP_FLAGS", 0,
	  "The wildcard flags passed to BPF_MAP_CREATE"},
	{ "max_entries", ARG_MAX_ENTRIES, "MAX_ENTRIES", 0,
	  "The wildcard max entries"},
...
const struct argp bench_wildcard_lookup_argp = {
	.options = opts,
	.parser = parse_arg,
};
...
static void setup(void)
{
	...
	ctx.skel = bpf_wildcard_lookup__open();
	if (!ctx.skel) {
		fprintf(stderr, "failed to open skeleton\n");
		exit(1);
	}

	bpf_map__set_max_entries(ctx.skel->maps.fw_hash, args.max_entries);
	bpf_map__set_key_size(ctx.skel->maps.fw_hash, args.key_size);
	bpf_map__set_value_size(ctx.skel->maps.fw_hash, 8);
	bpf_map__set_map_flags(ctx.skel->maps.fw_hash, args.map_flags);
	...
	ret = bpf_wildcard_lookup__load(ctx.skel);
	if (ret) {
		bpf_wildcard_lookup__destroy(ctx.skel);
		fprintf(stderr, "failed to load map: %s", strerror(-ret));
		exit(1);
	}

	/* fill in the hash_map */
	map_fd = bpf_map__fd(ctx.skel->maps.fw_hash);
...
static void wildcard_report_final(struct bench_res res[], int res_cnt)
{
...
const struct bench bench_bpf_wildcard_lookup = {
	.name = "bpf-wildcard-lookup",
	.argp = &bench_wildcard_lookup_argp,
	.validate = validate,
	.setup = setup,
	...
	.report_final = wildcard_report_final,
};
