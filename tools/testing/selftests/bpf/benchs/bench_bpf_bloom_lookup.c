...
#include <argp.h>
#include "bench.h"
#include "bpf_bloom_lookup.skel.h"
#include "bpf_util.h"

/* BPF triggering benchmarks */
static struct ctx {
	struct bpf_bloom_lookup *skel;
} ctx;
...
const struct argp bench_bloom_lookup_argp = {
	.options = opts,
	.parser = parse_arg,
};
...
static void setup(void)
{
	...
	ctx.skel = bpf_bloom_lookup__open();
	if (!ctx.skel) {
		fprintf(stderr, "failed to open skeleton\n");
		exit(1);
	}

	bpf_map__set_max_entries(ctx.skel->maps.bloom_map_bench, args.max_entries);
	bpf_map__set_value_size(ctx.skel->maps.bloom_map_bench, args.key_size);
	bpf_map__set_map_flags(ctx.skel->maps.bloom_map_bench, args.map_flags);
	//bpf_map__set_map_extra(ctx.skel->maps.bloom_map_bench, 5);
	...
	ret = bpf_bloom_lookup__load(ctx.skel);
	if (ret) {
		bpf_bloom_lookup__destroy(ctx.skel);
		fprintf(stderr, "failed to load map: %s", strerror(-ret));
		exit(1);
	}

	/* fill in the hash_map */
	int map_fd = bpf_map__fd(ctx.skel->maps.bloom_map_bench);
	for (u64 i = 0; i < args.nr_entries; i++) {
		patch_key(i, ctx.skel->bss->key);
		bpf_map_update_elem(map_fd, NULL, ctx.skel->bss->key, BPF_ANY);
	}
...
static void bloom_report_final(struct bench_res res[], int res_cnt)
{
...
const struct bench bench_bpf_bloom_lookup = {
	.name = "bpf-bloom-lookup",
	//.agrp = &bench_bloom_lookup_argp,
	.validate = validate,
	.setup = setup,
	...
	.report_final = bloom_report_final,
};
