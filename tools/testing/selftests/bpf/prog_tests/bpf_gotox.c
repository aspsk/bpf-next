...
static void allow_offsets(struct bpf_insn *insns, __u32 insn_cnt, int off1, int off2, int off3)
{
	LIBBPF_OPTS(bpf_test_run_opts, topts);
	int prog_fd, err;
	char s[128] = "";
...
static void reject_offsets(struct bpf_insn *insns, __u32 insn_cnt, __u32 off1, __u32 off2, __u32 off3)
{
	int prog_fd;
	...
	for (off1 = 0; off1 < 6; off1++)
		for (off2 = 0; off1 + off2 < 6; off2++)
			for (off3 = 0; off1 + off2 + off3 < 6; off3++)
				allow_offsets(insns, ARRAY_SIZE(insns), 8 * off1, 8 * off2, 8 * off3);
