...
static int load_test_prog(int map_fd, int *bind_fd_array, int bind_fd_array_cnt)
{
	...
	union bpf_attr attr = {
		...
		.bind_fd_array = ptr_to_u64(bind_fd_array),
		.bind_fd_array_cnt = bind_fd_array_cnt,
	};

	return syscall(__NR_bpf, BPF_PROG_LOAD, &attr, sizeof(attr));
}

static bool check_expected_map_ids(int prog_fd, int expected)
{
	__u32 map_ids[16];
	__u32 nr_map_ids = 16;
	int err;
...
static void check_bind_fd_array(void)
{
	int extra_map_fds[8];
	int prog_fd;
	int map_fd;
	...
	if (ASSERT_GE(prog_fd, 0, "BPF_PROG_LOAD"))
		return;
	if (!check_expected_map_ids(prog_fd, 1))
		return;
	close(prog_fd);
	...
	if (ASSERT_GE(prog_fd, 0, "BPF_PROG_LOAD"))
		return;
	if (!check_expected_map_ids(prog_fd, 3)) // XXX save map_ids
		return;
	...
	if (ASSERT_GE(prog_fd, 0, "BPF_PROG_LOAD"))
		return;
	if (!check_expected_map_ids(prog_fd, 3))
		return;
	/* maps should still exist when file descriptors are closed */
	close(extra_map_fds[0]);
	close(extra_map_fds[1]);
...
void test_prog_load_bind_fd_array(void)
{
	if (test__start_subtest("bind_fd_array"))
		check_bind_fd_array();
}
