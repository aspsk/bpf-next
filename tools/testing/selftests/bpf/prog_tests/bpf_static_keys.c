...
static inline int bpf_static_key_update(int map_fd, __u32 on)
{
	union bpf_attr attr = {
		.static_key.map_fd = (__u32)map_fd,
		.static_key.on = on,
	};

	return syscall(__NR_bpf, BPF_STATIC_KEY_UPDATE, &attr, sizeof(attr));
}
...
void test_bpf_static_keys(void)
{
	...
	if (0 && test__start_subtest("check_syscall"))
		check_syscall();
}
