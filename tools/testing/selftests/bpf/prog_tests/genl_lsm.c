...
static int nlctrl_request(int fd, __u8 cmd, bool dump)
{
	...
	req.genl.cmd = cmd;
	req.genl.version = 2;
	if (addattrstrz(&req.nlh, sizeof(req), CTRL_ATTR_FAMILY_NAME,
			NLCTRL_FAMILY_NAME))
		return -EMSGSIZE;
...
/* a request to a family which is not nlctrl, expected to succeed */
...
static void test_doit(struct genl_lsm *skel, int fd)
{
	...
	if (!ASSERT_OK(err, "allow_getfamily"))
		return;

	skel->bss->allow = false;
	err = nlctrl_request(fd, CTRL_CMD_GETFAMILY, false);
	ASSERT_EQ(err, -TEST_DENY_ERRNO, "deny_getfamily");
}
...
static void test_dump(struct genl_lsm *skel, int fd)
{
	...
	skel->bss->allow = false;
	err = nlctrl_request(fd, CTRL_CMD_GETPOLICY, true);
	ASSERT_EQ(err, -TEST_DENY_ERRNO, "deny_getpolicy_dump");
}

/*
 * CTRL_CMD_GETFAMILY is registered twice, once with GENL_CMD_CAP_DO and once
 * with GENL_CMD_CAP_DUMP, so the same command reaches the hook with two
 * different nlmsg_flags. That isolates the flags from the command.
 */
static void test_nlmsg_flags(struct genl_lsm *skel, int fd)
{
	...
	ASSERT_EQ(err, -TEST_DENY_ERRNO, "deny_dump_flags");

	/*
	 * Same command without NLM_F_DUMP, so the flags no longer match.
	 * Require it to succeed rather than merely to avoid our errno, so a
	 * request which never reached the hook cannot pass this.
	 */
	err = nlctrl_request(fd, CTRL_CMD_GETFAMILY, false);
	ASSERT_OK(err, "doit_flags_not_matched");
}

/* the policy names nlctrl, so a request to any other family is untouched */
static void test_other_family(struct genl_lsm *skel, int fd, __u16 other_id)
{
	...
	err = nlctrl_request(fd, CTRL_CMD_GETFAMILY, false);
	if (!ASSERT_EQ(err, -TEST_DENY_ERRNO, "nlctrl_denied"))
		return;

	/* has to succeed, otherwise it might not have reached the hook at all */
	err = other_family_request(fd, other_id);
	ASSERT_OK(err, "other_family_not_denied");
}

/*
 * The hook is handed the netns the request is being processed in, so a policy
 * can be scoped to one. Target the namespace the test creates and check that
 * the very same request is denied inside it and allowed outside it.
 */
static void test_netns(struct genl_lsm *skel, int fd)
{
	...
	/* a netlink socket belongs to the namespace it was opened in */
	ns_fd = genl_open(0);
	if (!ASSERT_OK_FD(ns_fd, "genl_open_in_netns"))
		goto out;
	...
	ASSERT_EQ(err, -TEST_DENY_ERRNO, "denied_in_target_netns");
...
void test_genl_lsm(void)
{
	...
	/* resolving is itself an nlctrl request, so do it before attaching */
	other_id = genl_resolve_family(fd, OTHER_FAMILY_NAME);
	if (!ASSERT_GT(other_id, 0, "resolve_other_family"))
		goto cleanup;
