...
static void test_doit(struct genl_bpf_hooks *skel, int fd)
{
	int err;
...
static void test_dump(struct genl_bpf_hooks *skel, int fd)
{
	int err;
...
static void test_nlmsg_flags(struct genl_bpf_hooks *skel, int fd)
{
	int err;
...
static void test_other_family(struct genl_bpf_hooks *skel, int fd, __u16 other_id)
{
	int err;
...
static void test_netns(struct genl_bpf_hooks *skel, int fd)
{
	...
	SYS_NOFAIL("ip netns del genl_bpf_hooks_ns");
	netns = netns_new("genl_bpf_hooks_ns", false);
	if (!ASSERT_OK_PTR(netns, "netns_new"))
		return;

	nstoken = open_netns("genl_bpf_hooks_ns");
...
void test_genl_bpf_hooks(void)
{
	struct genl_bpf_hooks *skel;
	int other_id, fd = -1;
	int err;

	skel = genl_bpf_hooks__open_and_load();
	if (!ASSERT_OK_PTR(skel, "genl_bpf_hooks__open_and_load"))
		return;
	...
	err = genl_bpf_hooks__attach(skel);
	if (!ASSERT_OK(err, "genl_bpf_hooks__attach"))
		goto cleanup;
