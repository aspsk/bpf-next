...
#include "ethtool_bpf_hooks.skel.h"
...
static void check_doit(struct ethtool_bpf_hooks *skel, int fd, __u16 family_id,
...
static void check_dump(struct ethtool_bpf_hooks *skel, int fd, __u16 family_id,
...
static void check_ioctl(struct ethtool_bpf_hooks *skel, __u32 ifindex)
{
...
static void check_ioctl_sub_cmd(struct ethtool_bpf_hooks *skel, __u32 ifindex)
{
...
void test_ethtool_bpf_hooks(void)
{
	__u32 netdevsim_ifindex;
	struct ethtool_bpf_hooks *skel = NULL;
	struct netns_obj *netns = NULL;
	struct nstoken *nstoken = NULL;
	int netdevsim_id = -1, family_id, fd = -1, err;

	SYS_NOFAIL("ip netns del ethtool_bpf_hooks_ns");
	netns = netns_new("ethtool_bpf_hooks_ns", false);
	if (!ASSERT_OK_PTR(netns, "netns_new"))
		goto out;

	nstoken = open_netns("ethtool_bpf_hooks_ns");
	...
	skel = ethtool_bpf_hooks__open_and_load();
	...
	err = ethtool_bpf_hooks__attach(skel);
...
out:
	if (fd >= 0)
		close(fd);
	ethtool_bpf_hooks__destroy(skel);
	if (netdevsim_id >= 0)
