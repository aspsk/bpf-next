			  ...
			  void **info_data, size_t *const info_data_sz)
{
	...
	holder.orig_to_jit_len = info->orig_to_jit_len;
	needed += info->orig_to_jit_len;
	...
	holder.orig_to_jit = ptr_to_u64(ptr);
	ptr += holder.orig_to_jit_len;
	  ...
	  char *filepath, bool opcodes, bool visual, bool linum)
{
	...
	if (filepath) {
		...
		dd.orig_to_jit = u64_to_ptr(info->orig_to_jit);
